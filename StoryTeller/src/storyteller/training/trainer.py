...
class DataParallelWrapper(nn.Module):
    """
    Wrapper for models to make them compatible with DataParallel.
    Returns only the loss tensor instead of complex dictionaries.
    """
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        """Forward pass that returns only loss for DataParallel compatibility."""
        outputs = self.model(
            input_ids=input_ids,
        ...
        # Return only the loss tensor for DataParallel to gather
        return outputs["loss"]
...
class Trainer:
    ...
    def __init__(
        ...
        if torch.cuda.is_available() and torch.cuda.device_count() > 1:
            print(f"Multi-GPU: Using DataParallel with {torch.cuda.device_count()} GPUs")
            # Wrap model to return only loss for DataParallel compatibility
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            with autocast(**autocast_kwargs):
                if self.is_data_parallel:
                    # DataParallelWrapper returns only loss
                    loss = self.model(
                        input_ids=input_ids,
                        attention_mask=attention_mask,
                        labels=labels,
                    )
                    # DataParallel already averages across GPUs
                    if loss.dim() > 0:
                        loss = loss.mean()
                    # Create dummy outputs dict for compatibility
                    outputs = {"loss": loss, "moe_loss": None}
                else:
                    # Normal forward pass
                    outputs = self.model(
                        input_ids=input_ids,
